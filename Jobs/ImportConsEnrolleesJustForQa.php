...
class ImportConsEnrolleesJustForQa implements ShouldQueue
{
    use Dispatchable;
    ...
    private function importFromEligibilityJob(Enrollee $enrollee, EligibilityJob $job)
    {
        ...
        $isSurveyOnlyUser = 'survey_only' === $job->batch->type;
        // Just another hack
        ...
        if (EligibilityBatch::CLH_MEDICAL_RECORD_TEMPLATE == $job->batch->type
            || $isSurveyOnlyUser) {
            $mr = new SurveyOnlyEnrolleeMedicalRecord($job, $enrollee->practice);
            ...
            $json = $isSurveyOnlyUser
                ? $mr->createFromSurveyOnlyUser($enrollee)
                : $mr->toJson();
            ...
            $mr = Ccda::create(
                [
                    ...
                    'mrn'                 => $isSurveyOnlyUser
                        ? $enrollee->id
                        : $job->data['patient_id'],

                    'json' => json_encode($json),
                    // Maybe uncomment this before testing
