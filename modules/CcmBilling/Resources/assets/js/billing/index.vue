                ...
                <template slot="CCM Problem Code(s)" slot-scope="props">
                    <div class="ccm-problem-codes">
                        <span class="blue pointer" style="overflow-wrap: break-word"
                              @click="showCcmModal(props.row)">{{
                                attestedCcmProblemCodes(props.row) || '&lt;Edit&gt;'
                            }}</span>
                    </div>
                </template>
...
<script>
    ...
    import NotificationsComponent
        from '../../../../../Sharedvuecomponents/Resources/assets/js/components/shared/notifications/notifications-event-based'
    ...
    export default {
        ...
        methods: {
            ...
            getCounts() {
                ...
                return this.axios
                    .get(rootUrl(`reports/monthly-billing/counts?practice_id=${this.selectedPractice}&date=${this.selectedMonth}&version=${this.version}`))
                    .then(response => {
            ...
            retrieve(isBackground) {
                ...
                this.axios
                    .post(this.url || rootUrl(`reports/monthly-billing/data`), {
                        practice_id: this.selectedPractice,
                        date: this.selectedMonth,
                        version: this.version
                    })
            ...
            parseError(err) {
                if (err.response && err.response.data && err.response.data.message) {
                    let msg = err.response.data.message + '<br/>';
                    for (let error in err.response.data.errors) {
                        if (!err.response.data.errors.hasOwnProperty(error)) {
                            continue;
                        }
                        msg += `${error}: ${err.response.data.errors[error]}<br/>`;
                    }
                    return msg;
                }
            ...
            openMonth() {
                ...
                return this.$http.post(rootUrl('reports/monthly-billing/open'), {
                    practice_id: this.selectedPractice,
                    date: this.selectedMonth,
                    version: this.version
                }).then(response => {
            ...
            closeMonth() {
                ...
                return this.$http.post(rootUrl('reports/monthly-billing/close'), {
                    practice_id: this.selectedPractice,
                    date: this.selectedMonth,
                    version: this.version
                }).then(response => {
