...
class CompareCurrentAndLegacyBillingDataPracticeChunk extends ChunksEloquentBuilderJob implements ShouldBeEncrypted
{
    ...
    {
        return $this->approvablePatientUsersQuery(Carbon::now()->startOfMonth())
    ...
    public function handle()
    {
        ...
        $this->getBuilder()->each(function ($patient) use ($servicesToCompare) {
            ...
            $dto = PatientMonthlyBillingDTO::generateFromUser($patient, Carbon::now()->startOfMonth());

            foreach ($servicesToCompare as $cs) {
                $toMatch['services'][$cs->display_name]['off'] = PatientIsOfServiceCode::fromDTO($dto, $cs->code);
            }

            BillingCache::setBillingRevampIsEnabled(true);

            foreach ($servicesToCompare as $cs) {
                $toMatch['services'][$cs->display_name]['on'] = PatientIsOfServiceCode::fromDTO($dto, $cs->code);
            }
            ...
            sendSlackMessage('#billing_alerts',
                "Warning! (From Billing Toggle Compare Job:) Patient ($patient->id), has the following code mismatches between billing revamp toggle states: {$mismatches}");
        });
