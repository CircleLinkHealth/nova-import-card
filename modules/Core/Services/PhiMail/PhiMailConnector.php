...
class PhiMailConnector
{
    ...
    public function check()
    {
        ...
        if ('STATUS' == substr($response, 0, 6)) {
            ...
            return \CircleLinkHealth\Core\Services\PhiMail\CheckResult::newStatus(
                $rExplode[1],
                $rExplode[2],
        ...
        if ('MAIL' == substr($response, 0, 4)) {
            ...
            return \CircleLinkHealth\Core\Services\PhiMail\CheckResult::newMail(
                $rExplode[1],
                $rExplode[2],
                $numAttach,
                $rExplode[4]
    ...
    public function send()
    {
        ...
        while (null != $response && 'OK' != $response) {
            ...
            switch ($rExplode[0]) {
                case 'ERROR':
                    $output[] = new \CircleLinkHealth\Core\Services\PhiMail\SendResult($rExplode[1], false, $rExplode[2]);
                    break;
                case 'QUEUED':
                    $output[] = new \CircleLinkHealth\Core\Services\PhiMail\SendResult($rExplode[1], true, $rExplode[2]);
                    break;
    ...
    public function show($messagePart)
    {
        ...
        if (0 == $messagePart) { //get attachment data
            ...
            for ($n = 0; $n < $numAttach; ++$n) {
                ...
                $ai[$n]       = new \CircleLinkHealth\Core\Services\PhiMail\AttachmentInfo($aFilename, $aMimeType, $aDescription);
            }
        }

        return new \CircleLinkHealth\Core\Services\PhiMail\ShowResult(
            $messagePart,
            $headers,
            $filename,
            $mimeType,
