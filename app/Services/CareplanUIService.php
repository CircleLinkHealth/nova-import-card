...
<h4>'.$item.'</h4>';
            ...
            foreach ($arrItem_value as $subKey => $arrSubKeyVal) {
                ...
                if ((1 == $item_pcp_id) || (9 == $item_pcp_id)
                    || 'Diagnosis / Problems to Monitor' == $item
                ) {
                        ...
                        switch ($value['ui_fld_type']) {
                            case 'SELECT':
                                ...
                                foreach ($arrOptions as $key => $optValue) {
                                    $selected = '';
                                    if (false !== strpos($inputVal, $optValue)) {
                                        $selected = ' SELECTED';
                                    }
