...
//@todo: At some point move all helper functions to a different file
    ...
    public function formatReportDataForView($report)
    {
        ...
        if (!self::filterAnswer($report->screenings)) {
            ...
            $countFamilyMembersWithSkinCancer = $familyMembersWithSkinCancer !== null ? count($familyMembersWithSkinCancer['family']) : 0;
...
//if no exception is catched
     ...
     * answers. If (array) && has items (string) && first answer is YES then will check each answer if NOT empty and it
     * will return answers. Else if (array) and has item (array) then return the value of the first item of each item
     * (array)
     *
     * @param $answers
     * @param $key
