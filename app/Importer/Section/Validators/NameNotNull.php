...
class NameNotNull implements SectionValidator
{
    ...
    {
        return ! (empty($item->translation_name) && empty($item->reference_title) && empty($item->text) && empty($item->product_name));
    }
    ...
    public function shouldValidate($item): bool
    {
        ...
        return $keys->contains('translation_name') || $keys->contains('reference_title') || $keys->contains('text') || $keys->contains('product_name');
    }
}
