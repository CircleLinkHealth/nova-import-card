...
class EnrollableSurveyCompleted implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $emailToString    = $this->getEmail($surveyAnswers['email']);
...
//        else return []
    ...
    private function getEmail(object $email)
    {
        $email = getStringValueFromAnswerAwvUser($email);
        // This will work. It is confusing.
        if ( ! is_string($email)) {
            $patientEmail              = $email->patientEmail;
            $patientAutoGeneratedEmail = $email->patientAutoGeneratedEmail;

            return empty($patientEmail) ? $patientAutoGeneratedEmail : $patientEmail;
        }
