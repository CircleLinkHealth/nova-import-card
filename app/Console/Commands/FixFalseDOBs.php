...
class FixFalseDOBs extends Command
{
    ...
    public function handle()
    {
        ...
        Patient::whereIn('id', $ids)->with('importedMedicalRecord')->whereHas(
            'user',
            function ($q) {
                $q->ofPractice(219);
            }
            ...
            function ($tmr) {
                $tmr->each(
                    function (Patient $patient) {
                        $e = Enrollee::whereMrn($patient->mrn_number)->whereHas('eligibilityJob')->with(
                            'eligibilityJob'
                        )->whereHas(
                            'eligibilityJob.batch.practice',
                            function ($q) {
                                $q->where('id', 219);
                            }
                        )->first();
