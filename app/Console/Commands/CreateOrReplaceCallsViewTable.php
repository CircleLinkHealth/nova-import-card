...
{
    /**
     * NOTE: Do not use this command, since migrations have been moved to a different module. SQL definition will be found in the migration itself.
     *
     * @var string
     */
    ...
    public function handle()
    {
        ...
        FROM 
            ...
            left join (select u.id as nurse_id, u.display_name as nurse from users u where u.deleted_at is null) as u2 on c.outbound_cpm_id = u2.nurse_id
            ...
            left join (select pi.user_id as patient_id, pi.last_contact_time as last_call, pi.no_call_attempts_since_last_success, pi.general_comment from patient_info pi where pi.deleted_at is null and pi.ccm_status = 'enrolled') as u4 on c.inbound_cpm_id = u4.patient_id
