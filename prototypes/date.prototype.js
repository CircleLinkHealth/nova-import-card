...
Date.prototype.toMediumDate = function (dayofweek) {
    ...
    d = d || new Date();
    var ret = "";
    if (dayofweek) ret += d.getDayOfWeek().slice(0, 3) + ", ";
    ret += d.getDate() + " " + d.getMonthOfYear().slice(0, 3) + ", " + d.getFullYear();
    return ret;
}

Date.prototype.toLongDate = function (weekDay) {
    var d = this;
    d = d || new Date();
    return (!weekDay ? (d.getDayOfWeek() + ", ") : "") + d.getDate() + " " + d.getMonthOfYear() + ", " + d.getFullYear();
}
...
Date.parseString = function (date) {
    if (!date || date == '') return new Date();
    else if (/\d+\-\d+\-\d+\T\d+\:\d+\:\d+/.test(date)) {
        var dsplit = date.split('T');
        var d = Date.GetDate(dsplit[0].replace(/-/g, '/'), "YYYY/MM/DD");
        var times = dsplit[1].split(':');
        return d.addHours(times[0]).addMinutes(times[1]);
    }
    else if (/\/Date\((-?)(\d+)\)\//.test(date)) return new Date(Number(date.toString().match(/(-?)(\d+)/gm)[0]));
