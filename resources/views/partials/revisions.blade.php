        ...
        <ul class="list-group">
            ...
            @foreach($revisions->take(5) as $history)
                @if($history->key == 'created_at' && !$history->old_value)
                    <li class="list-group-item">{{ $history->userResponsible()->user_login }}[{{ $history->userResponsible()->ID }}] created new resource {{ $history->revisionable_type }}[{{ $history->revisionable_id }}] on <em>{{ $history->created_at }}</em></li>
                @else
