    ...
    (function ($, document) {
        ...
        var submitViaAjax = function () {
            ...
            $.post($action.val(), $(this).serialize()).then(function (res) {
                $(document).trigger("complex-ccm-form-submit", !!res.is_ccm_complex) //raise event with the value of the response (true/false)
                //console.log("complex-ccm-badge-form-response", res);
            }).catch(function (err) {
                //console.error(err);
            })
            return false;
