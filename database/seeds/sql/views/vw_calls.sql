...
select 
	...
	(select id from practices where id = (select program_id from users where id = calls.inbound_cpm_id limit 1)) as practice_id,
	(select display_name from practices where id = (select program_id from users where id = calls.inbound_cpm_id limit 1)) as practice_name,
