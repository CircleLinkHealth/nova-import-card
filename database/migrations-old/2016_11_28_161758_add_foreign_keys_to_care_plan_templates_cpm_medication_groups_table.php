...
class AddForeignKeysToCarePlanTemplatesCpmMedicationGroupsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('care_plan_templates_cpm_medication_groups', function (Blueprint $table) {
            $table->foreign(
                'cpm_instruction_id',
                'care_plan_templates_cpm_medication_groups_instrction_foreign'
            )->references('id')->on('cpm_instructions')->onUpdate('CASCADE')->onDelete('SET NULL');
            $table->foreign(
                'cpm_medication_group_id',
                'cpm_medi_groups_rel_foreign'
            )->references('id')->on('cpm_medication_groups')->onUpdate('CASCADE')->onDelete('CASCADE');
            $table->foreign(
                'care_plan_template_id',
                'cpt_id_cpt_id_rel_foreign'
