...
class PatientUnsuccessfulCallNotification extends Notification
{
    ...
    private function getMailMessage(User $patient): MailMessage
    {
        ...
        $fromAddress = ! empty(config('mail.from-with-inbound.address')) ? config('mail.from-with-inbound.address') : config('mail.from.address');
        $fromName    = config('mail.from-with-inbound.name');
