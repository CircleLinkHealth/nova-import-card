...
var UploadedCcdsPanel = Vue.extend({
    ...
    methods: {
        importCcds: function importCcds() {
            ...
            this.$http.post('/ccd-importer/import', { ccdaIds: this.okToImport }, function (data, status, request) {
                ...
                for (var i = 0; i < data.imported.length; i++) {
                    $('#checkbox-' + data.imported[i].qaId).html('<a target="_blank" href="https://' + window.location.href.match(/:\/\/(.[^/]+)/)[1] + '/manage-patients/' + data.imported[i].userId + '/summary' + '"><b style="color: #06B106">Go to CarePlan</b></a>');
                }
        ...
        fetchImportedInfo: function fetchImportedInfo(event) {
    ...
    methods: {
        submitForm: function submitForm() {
            ...
            this.$http.post('/ccd-importer/demographics', payload).then(function (response) {
                alert('It seems like it went well. Click back and import this CCD. You will NOT see the changes you made ' + 'reflected on the summary screen. Your changes will be seen in the careplan.' + 'Expect more progress soon.');
            }, function (response) {
