...
class ImportService
{
    ...
    public function createTabularMedicalRecordAndImport($row, Practice $practice)
    {
        ...
        if (array_key_exists('primary_phone', $row) && array_key_exists('primary_phone_type', $row)) {
            if (Illuminate\Support\Str::contains(strtolower($row['primary_phone_type']), ['cell', 'mobile'])) {
                $row['cell_phone'] = $row['primary_phone'];
            } elseif (Illuminate\Support\Str::contains(strtolower($row['primary_phone_type']), 'home')) {
                $row['home_phone'] = $row['primary_phone'];
            } elseif (Illuminate\Support\Str::contains(strtolower($row['primary_phone_type']), 'work')) {
                $row['work_phone'] = $row['primary_phone'];
            }
        }

        if (array_key_exists('alt_phone', $row) && array_key_exists('alt_phone_type', $row)) {
            if (Illuminate\Support\Str::contains(strtolower($row['alt_phone_type']), ['cell', 'mobile'])) {
                $row['cell_phone'] = $row['alt_phone'];
            } elseif (Illuminate\Support\Str::contains(strtolower($row['alt_phone_type']), 'home')) {
                $row['home_phone'] = $row['alt_phone'];
            } elseif (Illuminate\Support\Str::contains(strtolower($row['alt_phone_type']), 'work')) {
    ...
    private function parseDOBDate($dob)
    {
        ...
        try {
            ...
            if (Illuminate\Support\Str::contains($dob, '/')) {
                $delimiter = '/';
            } elseif (Illuminate\Support\Str::contains($dob, '-')) {
                $delimiter = '-';
            }
